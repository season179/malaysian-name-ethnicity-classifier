    ...
    try {
      ...
      if (!res.ok) {
        ...
        if (res.status === 429 || (res.status >= 500 && res.status < 600)) {
          throw new Error(`Retryable OpenRouter API error: ${res.status} ${res.statusText}`);
        }
        // Non-retryable error
        return {
      ...
      if (
        attempt < maxRetries &&
        (err?.message?.includes('Retryable') || err?.name === 'FetchError' || err?.message?.includes('network'))
      ) {
        await new Promise((r) => setTimeout(r, retryDelayMs * Math.pow(2, attempt)));
        continue;
      }

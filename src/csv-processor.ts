...
export async function* readCsvStream(
    filePath: string
): AsyncGenerator<z.infer<typeof InputCsvRowSchema>> {
    ...
    for await (const chunk of stream) {
        ...
        for (const line of lines) {
            ...
            if (values.length !== headers.length) {
                console.warn(
                    `Skipping malformed row ${rowNum}: column count mismatch.`
                );
                continue;
            }
            ...
            try {
                ...
                console.warn(
                    `Row ${rowNum} failed validation: ${(err as Error).message}`
                );
                continue;
            }
        }
    ...
    if (buffer && headers) {
        ...
        if (values.length === headers.length) {
            ...
            try {
                ...
                console.warn(
                    `Final row failed validation: ${(err as Error).message}`
                );
            }
        }
    }
...
export async function processCsvInBatches({
    ...
    try {
        ...
        console.info(
            `CSV processing completed. Total records processed: ${processed}`
        );
    } catch (err) {
...
export async function writeCsvStream(
    outputFile: string,
    rows: AsyncIterable<OutputCsvRow>
) {
...
export async function processCsvPipeline({
    ...
    batchSize?: number;
    transformFn?: (row: InputCsvRow) => OutputCsvRow | Promise<OutputCsvRow>;
    batchTransformFn?: (batch: InputCsvRow[]) => Promise<OutputCsvRow[]>;
    onProgress?: (processed: number, total?: number) => void;
}) {
    // Streaming write: open file, write header, then append each batch after processing
    const headers = Object.keys(OutputCsvRowSchema.shape);
    // Write header at the beginning (overwrite file)
    await Bun.write(outputFile, headers.join(",") + "\n");
    let batch: InputCsvRow[] = [];
    let processed = 0;
    for await (const row of readCsvStream(inputFile)) {
        batch.push(row);
        if (batch.length >= batchSize) {
            let outputs: OutputCsvRow[] = [];
            if (batchTransformFn) {
                outputs = await batchTransformFn(batch);
            } else if (transformFn) {
                outputs = await transformAndValidateBatch(batch, transformFn);
            }
